...
import cn.zhaoqin.seckill.mq.MQSender;
import cn.zhaoqin.seckill.mq.SeckillMessage;
import cn.zhaoqin.seckill.redis.GoodsKey;
...
//        User user = redisService.get(UserKey.getByName, loginToken, User.class);
    ...
    @ResponseBody
    public Result<Long> miaoshaResult(@RequestParam("goodsId") long goodsId, HttpServletRequest request, HttpSession session) {
//        String loginToken = CookieUtil.readLoginToken(request);
//        User user = redisService.get(UserKey.getByName, loginToken, User.class);
        User user = redisService.get(UserKey.getByName, session.getId(), User.class);
        if (user == null) {
...
//    @AccessLimit(seconds=5, maxCount=5, needLogin=true)
    ...
    public Result<String> getMiaoshaPath(HttpServletRequest request, User user,
                                         @RequestParam("goodsId") long goodsId, HttpSession session) {

//        String loginToken = CookieUtil.readLoginToken(request);
//        user = redisService.get(UserKey.getByName, loginToken, User.class);
        user = redisService.get(UserKey.getByName, session.getId(), User.class);
        if (user == null) {
            return Result.error(CodeMsg.USER_NO_LOGIN);
        }
